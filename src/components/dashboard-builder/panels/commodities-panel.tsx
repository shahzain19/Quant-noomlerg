...
export function CommoditiesPanel() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/markets?type=commodity");
  ...
  if (!data.length) return <div className="flex items-center justify-center h-full text-zinc-500 text-xs">No commodities</div>;
